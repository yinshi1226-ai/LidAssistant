...
LIDASSIST_TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/lidassist-grant.XXXXXX")"
...
cat > "$LIDASSIST_TMP_DIR/root.sh" <<EOF
#!/bin/sh
set -e
...
osascript -e "do shell script \"/bin/sh $LIDASSIST_TMP_DIR/root.sh\" with administrator privileges" >/dev/null
echo "✓ 免密授权与看门狗已安装"
